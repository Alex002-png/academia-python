...
- **[CERRADO] La expansión de calidad (16→27 laboratorios) recibió una segunda ronda de auditoría adversarial** (3 auditores independientes: coherencia/densidad, cumplimiento DOC-12, corrección técnica) sobre los 11 laboratorios nuevos. Hallazgos reales encontrados y corregidos: una cita mal enlazada (Denil et al. en M2.T1b apuntaba al arXiv ID de otro paper), un ejemplo numérico que no demostraba lo que el texto afirmaba (FP16 vs. BF16 en M2.T1b daban resultados idénticos para el valor original), una cifra ("88% de desperdicio" en M4.T2b) no verificable de forma independiente en el texto del paper de PagedAttention y por tanto retirada, una inconsistencia de prosa contra el propio cálculo del laboratorio (M3.T4b), un recurso faltante (M4.T2b tenía solo 1, mínimo DOC-12 §2.12 es 2), un swap de instrumentos en las capacidades de dominio (M3.T4b), y el hallazgo más sistémico: 6 de los 11 `errorVivo` nuevos eran tareas de predicción o autorrevisión, no errores genuinamente reproducibles (violación de DOC-12 §2.5bis) — rediseñados como comandos reales con resultado verificado por ejecución antes de escribirse. Detalle completo en el commit `d0d7e66` de la rama `nivel/n10`. No quedan hallazgos de esta ronda sin aplicar.
