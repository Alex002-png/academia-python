...
**Addendum post-cierre (2026-07-22, expansión de práctica):** tras el cierre institucional documentado más abajo, el usuario pidió explícitamente profundizar cada tema del nivel con un banco grande de ejercicios ("por cada tema, 25 ejercicios"). Se añadió el campo `practicaExtendida` (25 ejercicios graduados por laboratorio — 8 Básico/9 Intermedio/5 Avanzado/3 Integrador) a los 22 laboratorios `modalidad:"real"` de `LEVEL8` — 550 ejercicios nuevos en total, verificados módulo por módulo (`node --check`, estructura `LEVEL8`, unicidad de `id`). El contenido teórico (fichas, `explicacion`, bibliografía) **no cambió** en esta expansión. Como consecuencia, la carga estimada se corrigió de nuevo vía **PATCH DOC-10 1.0.5**, de "~170h" a "~360-510h, estimado ~435h (~4 meses)" — el detalle completo de esta expansión y su metodología de estimación está en `docs/syllabus/syl-n8.md` §10. **Toda cifra de "~170h" citada en el cuerpo de este informe (secciones 1-10, sin modificar debajo) está superada por esta corrección** y debe leerse junto con este addendum, no en su lugar.
