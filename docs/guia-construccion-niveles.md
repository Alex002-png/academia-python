...
- **Tercer hallazgo de costo, distinto en naturaleza a los dos anteriores (2026-07-22, hallazgo real al construir N4.M4.T2 — validación cruzada):** `cross_val_score`/`cross_validate` no son gratis por dentro — verificado empíricamente que el costo POR FIT interno de `cross_val_score` es notablemente mayor (~2× en las mediciones reales de este proyecto) que llamar `.fit()` a mano sobre el mismo modelo, por la maquinaria adicional de validación de parámetros/scoring que ejecuta en cada iteración. Esto se vuelve crítico al combinar validación cruzada con **muchos fits internos**: `LeaveOneOut()` sobre un dataset de tamaño `n` hace `n` fits (n=20 ya cuesta ~230 000 pasos con un modelo simple como `LogisticRegression`, cerca del límite); y **repetir `cross_val_score` varias veces en el mismo script** (ej. un loop probando distintos `random_state` de un `KFold(shuffle=True)`) ACUMULA el costo dentro de la MISMA ejecución trazada — 10 repeticiones de un 5-fold ya blindron el límite de 300 000 pasos en las pruebas reales de esta lección. **Regla práctica: si tu ejercicio necesita repetir validación cruzada varias veces en el mismo script, o combinar `LeaveOneOut` con otra llamada de `cross_val_score` en el mismo ejercicio, prefiere un loop MANUAL con `KFold(...).split()` + `.fit()`/`.score()` directo (más barato, y además hace transparente el mecanismo interno, coherente con enseñar "cómo funciona por dentro") en vez de apilar múltiples llamadas a `cross_val_score` dentro del mismo `check()`.** Verifica el costo real ANTES de fijar cuántas repeticiones/folds usa tu ejercicio (mismo patrón de verificación empírica que los dos hallazgos anteriores) — este no requirió fix de motor (no es un bug, es un costo real e inherente de la abstracción), solo disciplina de diseño de ejercicio.
