...
**Donde los 4 auditores convergieron, de forma independiente, en encontrar debilidades reales:** (1) un tema (M2.T4, descenso de gradiente) tenía 8 ejercicios contra 10 de sus hermanos — encontrado por los 4 auditores con números casi idénticos, ahora corregido añadiendo 2 ejercicios reales y verificados; (2) el patrón de `hints` que regalan la solución completa en vez de dar un empujón parcial — señalado como observación menor en N1, como hallazgo limitado en N2, y ahora encontrado por los 4 auditores de N3 como el patrón dominante de prácticamente todo el nivel (estimaciones entre 79% y 92% de los ~285 ejercicios regulares), un salto de escala real, resuelto después en la misma jornada por instrucción del Director (ver actualización abajo); (3) un error factual real y aislado (`n3m2t4c` afirmaba "cierra T4 y M2" cuando M2 continúa 7 días más) — encontrado y corregido en el propio proceso de auditoría; (4) desincronización real entre el ritmo de construcción del contenido y el aparato institucional que lo documenta (Blueprint y encabezado del syllabus) — dos casos concretos, ya corregidos.

**Actualización posterior (2026-07-21, misma jornada) — el hallazgo #2 (hints) fue resuelto por instrucción directa del Director ("resuelve, y súbelo").** Se reescribieron progresivamente las pistas de los 132 ejercicios reales que calificaban (starter con `pass`/blanco real Y primer hint ya literal), mediante 8 agentes independientes en worktrees aislados (uno por porción de módulo, para editar el mismo archivo en paralelo sin conflictos), cada uno con instrucción explícita de anteponer un hint conceptual específico (nunca genérico) sin tocar `check()`/`starter`/`d`/`t`, y de no tocar `retoFinal` (ya verificados como correctos) ni ejercicios de tipo "código ya completo, ejecuta y observa" (un tipo distinto, sin el mismo problema). Las 8 ramas se fusionaron sin ningún conflicto real (0 de 8 merges tuvo colisión). Re-verificado íntegramente tras la fusión: `node --check` limpio (array `LEVEL3` y bloque `&lt;script&gt;` completo); los 4 harnesses de módulo (M1 85/85, M2 72/72, M3 72/72, M4 55/55) confirman que ningún `check()` cambió de comportamiento; 895 ids, 0 duplicados. Ver commit y EVT correspondiente para el detalle completo.
...
**🟢 Corregido (2026-07-21, PATCH DOC-10 1.0.3, autorizado por el Director) — DOC-10 §6 no reflejaba la propia revisión honesta de duración de SYL-N3.** SYL-N3 §1 declara con transparencia que la densidad ampliada de M1-M3 sube la duración real de ~3 a ~4,6 meses (~20 semanas) — pero DOC-10 §6, el documento de mayor autoridad que un lector consultaría primero, seguía mostrando "~3 meses · ~300h" sin ninguna nota. Mismo patrón que un hallazgo ya señalado en la auditoría de N1 (SYL-N1 §4 no reflejaba una decisión ya tomada). Este comité había decidido NO corregir DOC-10 directamente por ser un documento Tier T1 ya sellado — quedó como recomendación explícita, y el Director la autorizó al ser consultado directamente. Corregido: "Carga estimada" de N3 §6 ahora dice "~4,6 meses · ~20 semanas".
...
| 2 | Hints regalan la solución completa en 79-92% de ~285 ejercicios regulares — tercera vez consecutiva (N1→N2→N3), ahora a escala de nivel completo | 🔴 Crítico/Importante (según auditor) | ✅ **Corregido** (132 ejercicios reescritos vía 8 agentes en worktrees aislados, 0 conflictos al fusionar, 4 harnesses de módulo confirman 0 regresiones) |
| 3 | Los 5 "estándares" narrativos de §5 (quiebre de intuición, etc.) nunca llegan a `index.html` — sin ruta de renderizado para modalidad Pyodide | 🟡 Importante | ✅ **Corregido** (decisión de diseño: disciplina de prosa, no campo nuevo — "quiebre de intuición" visible en el título de la tema donde ocurre por módulo; los otros 4 estándares añadidos a la prosa de `intro` de los 4 cierres reales de módulo) |
...
| 10 | DOC-10 §6 no reflejaba la duración real (~4,6 meses) ya declarada honestamente en SYL-N3 §1 | 🟡 Importante | ✅ **Corregido** (PATCH DOC-10 1.0.3, autorizado por el Director) |
