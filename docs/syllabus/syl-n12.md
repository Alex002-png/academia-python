...
| **Versión / Estado** | **1.0.0-candidato (densidad de publicación)** · Flujo institucional de 9 pasos completo: diseño (Paso 1) + M1-M5 (20 temas) + Capstone ET5 (Paso 4) + Paso 8 (compuertas + banco de examen) + Paso 9a (herencias finales, N12 confirmado hoja terminal) + Auditoría Integral inicial (4 auditores) + Paso 9b Auditoría Adversarial inicial (2 comités, 6 ataques, 3 con mérito corregidos) + Informe Final de Nivel completos sobre la versión corta original. **Ronda de expansión de densidad (2026-07-22):** los 20 temas fueron expandidos de 1-2 días/tema a la densidad real que cada uno sostiene honestamente (M1: 47 días/4 temas, M2: 30 días/5 temas, M3: 22 días/5 temas modalidad DOC-12 real, M4: 15 días/4 temas simulacro en inglés, M5: 6 días/2 temas — 120 días totales en `LEVEL12`), verificando cada valor numérico nuevo con ejecución Python real y cada hecho técnico nuevo (AWS, ML infra) vía WebSearch contra documentación oficial. **Segunda ronda de auditoría sobre el contenido expandido:** Auditoría Integral (5 agentes independientes, uno por módulo, contra los 4 criterios de aceptación DOC-11) — hallazgos reales corregidos: 12 ejercicios "Transferencia" en M1/M2 cuyo `starter` incluía la solución y la llamada ya resueltas (0% de esfuerzo real exigido), 1 comando AWS roto por doble escape, 1 error técnico de familia de GPU, 1 inconsistencia de security groups AWS, 1 cita cronológicamente invertida entre módulos, 1 hito del capstone que no exigía la disciplina de control de variables que el propio M5.T2 enseña, más varios hallazgos menores (notas de free tier, advertencias de costo faltantes, mixed-language). Auditoría Adversarial (2 agentes, exploits/contradicciones + gobernanza-estándar-de-calidad) — hallazgos reales corregidos: 7 ejercicios de "investigar y defender" cuyo `check()` solo verificaba longitud de comentario sin contenido semántico (explotables con texto de relleno), 5 pares de ejercicios del mismo día en M1.T2 sin variación pedagógica real (padding, DOC-11 v2.0.1), y 1 fórmula de memoria de entrenamiento de GPU incompleta (omitía gradientes y copia maestra fp32, subestimando memoria real en 37.5%). **Candidato a v1.0.0 con 1 reserva nombrada y acotada, sin cambios** (Ataque 4 original: M4 fuerza el esquema DOC-12 de terminal sobre un simulacro oral — no bloquea que un estudiante complete N12, requiere decisión del Director sobre DOC-12 Tier T1) — pendiente del veredicto definitivo del Director, nunca autodeclarado (guía §12) |
...
| **Historial** | 0.1.0-draft (2026-07-21): Documento de Diseño. Leídos íntegros: guía de construcción de niveles, ficha de misión N12, DOC-10 (mapa completo + §9/§10), DOC-01 (C-N12-01…06 y matriz de cobertura), DOC-02 (rúbricas RM-01…08 y matriz canónica — hallazgo clave: **RM-08/TS-01 y TD-03 ya existían, pre-aprobados, exactamente para M4 y C-N12-06** — no fue necesario inventar el formato, solo instanciarlo), DOC-03 (14 momentos, 8 elementos de §3.F, rol Entrevistador ya definido en §3.D), DOC-11 y DOC-12 completos, y las 5 fichas de misión N7-N11 para tener el panorama completo de lo que este nivel sintetiza. Investigación real verificada por WebSearch (no de memoria): *System Design Interview* Vol. 1 (Xu, tabla de contenidos confirmada — 16 capítulos, marco de 4 pasos), *Machine Learning System Design Interview* (Aminian & Xu) y *Generative AI System Design Interview* (Aminian & Sheng, ByteByteGo) confirmados como títulos reales con tablas de contenido verificadas, AWS SAA-C03 (4 dominios confirmados: Seguridad 30%/Resiliencia 26%/Rendimiento 24%/Costos 20%, AWS Skill Builder como ruta gratuita oficial), *The Ultra-Scale Playbook* (Hugging Face, paralelismo de datos/tensor/pipeline/contexto, ~4000 experimentos reales en hasta 512 GPUs), *Cracking the Coding Interview* (McDowell, sección conductual con método STAR confirmada), y los blogs de ingeniería reales de OpenAI (`openai.com/news/engineering`) y Anthropic (`anthropic.com/engineering`) verificados como fuentes vivas y activas — ver §6 para el detalle completo con motivo de cada fuente. **1.0.0-candidato densidad de publicación (2026-07-22):** los 20 temas expandidos de 1-2 días a su profundidad honesta (120 días totales), tema por tema, con verificación Python real de cada nuevo valor numérico y WebSearch contra documentación oficial de cada nuevo hecho técnico (AWS: NAT Gateway, ALB health checks, Auto Scaling target tracking, S3 storage classes, DynamoDB capacity modes, RDS Multi-AZ, SageMaker/Serverless/Bedrock, IAM roles/STS, Secrets Manager, KMS; ML infra: memoria de entrenamiento con Adam). Seguida de dos rondas completas de auditoría sobre el contenido expandido — Auditoría Integral (5 agentes paralelos, uno por módulo, contra los 4 criterios de aceptación DOC-11) y Auditoría Adversarial (2 agentes, exploits de `check()`/contradicciones + gobernanza-estándar-de-calidad) — con todos los hallazgos reales corregidos directamente en `index.html` (ver tabla de Control arriba para el resumen; commits individuales en la rama `nivel/n12` documentan cada corrección con su justificación técnica completa). |

---

## 1. Tabla resumen

| Módulo | Temas | Días totales | Días/tema (real, post-expansión) | Competencias | Modalidad dominante | Quiebre de intuición |
|---|---|---|---|---|---|---|
| M1 · System design | 4 | 47 | 8-18, variable por tema | C-N12-02 | Método + simulacro (RM-08/TS-01) | T1: no existe "la" arquitectura correcta — existe la que mejor paga las restricciones dadas, y un candidato se evalúa por cómo razona bajo repregunta, no por la primera respuesta |
| M2 · Arquitectura de sistemas de IA | 5 | 30 | 4-9, variable por tema | C-N12-01 | Diseño/decisión (DOC-11-lite + escrito) | T2: la razón para paralelizar nunca es "tener más GPUs" — es que un solo modelo o un solo lote ya no caben en la memoria o el tiempo de una sola GPU, y cada forma de partirlo paga un costo de comunicación distinto |
| M3 · Cloud aplicado (AWS) | 5 | 22 | 3-5 | (sirve a C-N12-01, práctica DOC-12) | DOC-12 dominante | T3: el costo real de un sistema de IA en producción casi nunca lo domina el cómputo de inferencia — lo domina lo que el estudiante nunca midió antes (egress, almacenamiento de vectores, llamadas redundantes) |
| M4 · Preparación profesional | 4 | 15 | 3-4 | C-N12-03 | Simulacro (RM-08/TS-01), en inglés | T3: una respuesta conductual "correcta" sin evidencia verificable (números, decisiones, consecuencias reales) no pasa una entrevista real — STAR sin "R" medible es una anécdota, no una prueba |
| M5 · Culminación | 2 | 6 | 2-4 | C-N12-04, C-N12-05, C-N12-06 | Rúbrica de criterio (RM-06) + capstone | Único: la "vigencia autónoma" no se demuestra citando el avance más reciente posible — se demuestra integrándolo de verdad, con una justificación que sobreviviría a la pregunta "¿por qué éste y no otro?" |
| Proyecto de nivel (capstone) | — | — | — | Todas (C-N12-01…06) | Proyecto + defensa final integradora (TD-03) | — |
| **Total** | **20 + capstone** | **120** | — | — | — | — |
...
*Densidad recalibrada por instrucción explícita del propietario del proyecto (2026-07-22, verbatim): "cada tema, 15 a 20 días y entre 25 ejercicios por cada tema", con el mandato de calidad "10/10 académico" que exige que cada tema pase la prueba de universidad Top 20 / Senior Engineer / entrevista difícil / vigencia a varios años (ver Informe Final de Nivel para el detalle completo del mandato). La cifra real por tema, verificada tema por tema en la construcción (no prometida antes de escribirla), quedó por debajo de 15-20 días de forma deliberada en los temas de síntesis/cierre (ej. M2.T5, M3.T5, M4.T4, M5.T1-T2) — el principio anti-relleno de DOC-11 v2.0.1 ("cada variación debe probar un caso que las anteriores no probaron") se aplicó sobre la instrucción literal de densidad cuando ambas entraron en tensión, confirmado explícitamente por dos rondas de auditoría (Integral + Adversarial) que encontraron y corrigieron el relleno real donde apareció (ver Control del documento arriba). 20 temas + capstone, 120 días totales, es la cifra real derivada de construir cada tema hasta su profundidad honesta, no una promesa previa.*
...
**Mismo patrón de hallazgo:** DOC-02 §4 ya resuelve esto también — `C-N12-04` y `C-N12-05` mapean contra **RM-06 · Proyecto** (evidencia [A][E], anclas de criterio: funciona de extremo a extremo / resiste el examen de un tercero / demuestra criterio de ingeniería más allá de lo pedido — nunca comparación de salida exacta) y plantilla **TP-01**. Esto confirma lo que la ficha de misión ya intuía: M5 no es una serie de lecciones con ejercicios propios en el sentido de M1-M4 — es, estructuralmente, **la antesala directa del capstone**, con 2 temas de guía real (§5, M5 — expandidos a 6 días totales en la construcción real, T1 con el criterio de selección/priorización/parada y T2 con el mecanismo de comparación controlada para distinguir integración real de cosmética) que enseñan el criterio de selección/justificación de un avance, y cuyo grueso de evidencia vive en los hitos del proyecto de nivel (§6), no en `ex[]` de M5.
