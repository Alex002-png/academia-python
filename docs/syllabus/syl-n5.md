...
8. **Explicación a dos audiencias** (C-N5-04, DOC-01 línea 125) — *principio original de diseño, corregido tras la auditoría integral (ver `docs/informes/n5-auditoria-integral.md`) para reflejar honestamente el contenido REAL construido, no la intención original*: en la práctica, la instancia con ejercicio verificado (`resumen_no_tecnico`, `check()` real) vive en M4.T3 y se ejercita en el capstone (hito 5); M1-M3 contribuyen una única pregunta reflexiva de comprensión (`n5m3t2`, "¿cuál de las dos métricas es más fácil de explicar a alguien sin formación técnica?"), sin ejercicio con `check()` propio. **Declarado honestamente: la práctica de C-N5-04 está CONCENTRADA en el cierre del nivel (M4.T3+capstone), no distribuida desde M1** — corrección aplicada aquí en vez de dejar una afirmación de diseño no verificada contra el contenido real, mismo principio de honestidad que M4 completo enseña.
9. **Nota metodológica sobre horas declaradas** (`docs/mision-n5.md` instruye explícitamente auditar la cifra de DOC-10 §7 contra el diseño real, mismo principio que N3 §3.7 — auditoría ejecutada en el cierre de la construcción, no al diseñar, siguiendo la propia disciplina anti-promesa-antes-de-verificar de §8): el contenido REAL construido son 27 días Pyodide (M1: 12, M3: 9, M4: 6) + 6 laboratorios DOC-12 en Colab (M2: 4, M3: 2) + capstone (~2-3 semanas declaradas). Estimando ~2.5h/día Pyodide (densidad comparable a N3) + ~1.5-2h efectivas por laboratorio DOC-12 (duración declarada 85-110min más fricción real de entorno nuevo) + ~15-20h de capstone, el contenido del Campus solo suma **~95-100h**; sumando el consumo real de la bibliografía oficial citada (DL Specialization son 5 cursos completos de Coursera, sustancialmente más largos que los ~58h de la especialización de N3 — estimación conservadora ~80-100h — más los videos de Karpathy ~10-15h), la cifra total ronda **~185-215h**, notablemente MENOR que las ~400h/~4 meses originales de DOC-10 §7. **A diferencia de N3 (donde la cifra original resultó subestimada), aquí DOC-10 parece haber sobreestimado** — declarado explícitamente como hallazgo pendiente de decisión del Director (mismo tratamiento que N3 dio a su propio hallazgo, sin editar DOC-10 directamente, Tier T1 ya sellado), no autocorregido en silencio.
...
**Hallazgo de la revisión:** las 4 competencias de N5 mapean 1:1 con los 4 módulos, igual que en N3 — cobertura completa, sin competencia huérfana. **Corrección tras la auditoría integral** (ver `docs/informes/n5-auditoria-integral.md` y §3.principio 8 ya corregido): a diferencia de lo declarado en el diseño original, C-N5-04 NO se practica de forma distribuida con ejercicio verificado desde M1 — M1.T4 tiene una defensa breve oral (sin campo propio en `index.html`) y M3.T2 una única pregunta reflexiva; la práctica con `check()` real está concentrada en M4.T3 y el capstone. Cobertura sigue siendo completa al cierre del nivel, pero la progresión declarada no coincidía con el contenido real — corregido aquí en vez de mantener la afirmación original sin verificar.
...
- **Convención declarada explícitamente (guía §9, hallazgo real ya documentado en N3.M3.T3 sobre `numpy.var`/`numpy.cov`):** las 3 varianzas de este ítem usan la convención POBLACIONAL (`ddof=0`, divide entre n, la misma que `varianza()` de M4.T1) — recalculadas con `ddof=1` (muestral) darían números distintos; el examinador nunca compara estos valores contra una librería con otra convención por defecto sin volver a verificar.
