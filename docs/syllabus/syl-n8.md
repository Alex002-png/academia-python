...
| **Versión / Estado** | **0.7.0-candidato a v1.0.0, sin reservas bloqueantes** · Flujo institucional de 9 pasos completo (Pasos 1-9b) más expansión de práctica post-cierre (§10) — investigación real verificada, decisiones de arquitectura documentadas, M1-M5 construidos en `LEVEL8` (22 laboratorios DOC-12, cada uno con banco `practicaExtendida` de 25 ejercicios — 550 en total), proyecto de nivel, compuertas, auditoría integral (4 auditores, 8 correcciones), Herencias Declaradas hacia N9, auditoría adversarial (2 revisores, 5 correcciones), Informe Final de Nivel (`docs/informes/n8-informe-final-de-nivel.md`). **Las 2 reservas del Paso 9b resueltas por instrucción directa del Director (2026-07-22)**: (1) carga estimada corregida vía PATCH administrativo DOC-10 1.0.4 y, tras la expansión de práctica, vía PATCH 1.0.5 (de "~5 meses·~500h" a "~6-7 semanas·~170h" a "~4 meses·~360-510h, estimado ~435h") — el Director instruyó explícitamente no inflar la cifra para sostener una expectativa histórica, y la cifra final es resultado de sumar contenido real, no de un objetivo elegido de antemano; (2) verificación empírica real de 8/22 `errorVivo` (36%, concentrados en M4) queda **registrada como limitación conocida, transparente, no bloqueante** — el Director prefirió explícitamente una limitación declarada a una verificación ficticia, con la comprobación real diferida al entorno definitivo. **Ninguna reserva bloquea la congelación del nivel — decisión explícita del Director.** |
...
| **Historial** | 0.1.0-draft (2026-07-21): Documento de Diseño — investigación real verificada por WebFetch sobre dominios oficiales (MCP, LangGraph, tool use de Anthropic/OpenAI, cursos DL.AI, papers arxiv sobre fallos de agentes — ver bibliografía completa en `n8-bibliografia-oficial.md`); 2 decisiones institucionales del Director confirmadas y documentadas (arquitectura de plataforma reutilizable N6-N12 por capas con interfaces estables, en vez de una aplicación vertical; agnosticismo de proveedor para N6-N12 con excepción única justificada de MCP); 22 temas diseñados sobre 5 módulos, todos DOC-12 salvo introducciones conceptuales acotadas. · 0.2.0 (2026-07-21): **M1-M5 construidos por completo en `LEVEL8`** — 22 laboratorios DOC-12 reales (M1:5, M2:5, M3:4, M4:3, M5:5), cada uno con código verificado contra las APIs oficiales investigadas, error inducido en vivo, diagnóstico de 5 columnas, y 2-5 recursos citados de la bibliografía verificada. `node --check` limpio y `LEVEL8` cargado en sandbox de Node en cada cierre de módulo; 0 ids duplicados en todo el Campus. `LEVELS`/`LEVEL_META` sin tocar en ningún commit. · 0.3.0 (2026-07-21): **Proyecto de nivel construido** (`n8proyecto`, modalidad `proyecto`, 5 hitos, `flujoDeGit`) y **compuertas diseñadas** (§7: tabla de cobertura de competencias sin hallazgo de omisión — las 4 competencias refuerzan en ≥2 puntos cada una, a diferencia del caso C-N2-07 de SYL-N2 — y banco de examen de 24 variantes, 8 ítems × 3, con criterio de rotación por parámetros de escenario en vez de cómputo numérico cerrado, formato correcto para una competencia de diseño de sistemas). · 0.4.0 (2026-07-21): **Auditoría integral ejecutada** — 4 auditores independientes sin coordinación entre sí (`docs/informes/n8-auditoria-integral.md`), 3 hallazgos moderados reales corregidos (C-N8-04 sembrado tempranamente en M1.T4; herencia saliente de límites de `SqliteSaver` hacia N9 documentada en M2.T2; badge de `NIVELES` corregido — anunciaba "CrewAI · A2A" sin artefacto curricular real — y frontera de protocolos multi-agente declarada explícitamente en M2.T4) más 5 correcciones menores. **Paso 9a completo**: coherencia de Herencia entrante de N7 reconfirmada sin discrepancia contra DOC-10 §8; Herencias Declaradas hacia N9 (H-N9-01…06) redactadas, incluyendo 2 brechas explícitamente declaradas (estado distribuido multi-proceso; protocolos de interoperabilidad agente-a-agente entre organizaciones) en vez de asumidas en silencio. · 0.5.0 (2026-07-22): **Auditoría adversarial ejecutada (Paso 9b)** — 2 revisores independientes (`docs/informes/n8-auditoria-adversarial.md`), 3 de 8 ataques sin fundamento (la arquitectura resiste: separación N7/N8/N9, aparato de ADRs respaldado por RM-05, `diagnostico` no regala solución), 5 con fundamento corregidos (patrón de Reflexión nombrado en M1.T1; 3 capas de la plataforma sin nombrar corregidas con nomenclatura explícita + quinto ADR en M5.T3; matiz de andamiaje Pyodide en §3.1; honestidad explícita añadida a 8 `errorVivo`; hallazgo de carga estimada documentado). **Informe Final de Nivel completo** (`docs/informes/n8-informe-final-de-nivel.md`), candidato a v1.0.0 con 2 reservas nombradas. · 0.6.0 (2026-07-22): **Ambas reservas resueltas por instrucción directa del Director** — PATCH DOC-10 1.0.4 aplicado (carga estimada corregida a ~170h, sin inflar); reserva de verificación empírica de `errorVivo` registrada explícitamente como limitación conocida y transparente, diferida al entorno definitivo, sin bloquear la congelación. Decisión explícita del Director: la propagación de las 2 decisiones institucionales (plataforma-no-aplicación, agnosticismo de proveedor) NO corresponde a esta ventana — queda para el documento maestro/ventana coordinadora. **SYL-N8 queda candidato a v1.0.0 sin reservas bloqueantes.** · 0.7.0 (2026-07-22): **Expansión de práctica post-cierre** — a petición explícita del usuario ("por cada tema, 25 ejercicios"), los 22 laboratorios `modalidad:"real"` reciben un campo nuevo `practicaExtendida` (550 ejercicios nuevos, 25 por laboratorio, graduados 8 Básico/9 Intermedio/5 Avanzado/3 Integrador), construidos y verificados módulo por módulo (`node --check`, estructura `LEVEL8` en sandbox, unicidad de `id` global) con 5 commits (uno por módulo M1-M5). PATCH DOC-10 1.0.5 aplicado: carga estimada corregida de "~170h" a "~360-510h (estimado ~435h)" sumando el banco nuevo (estimado por categoría de dificultad, no medido con estudiantes reales) al núcleo ya verificado (labs+proyecto, ≈169h sin cambio). Contenido teórico (fichas, `explicacion`, `contexto`, bibliografía) sin cambios en esta expansión — la elevación académica ya se hizo en 0.5.0-0.6.0. |
...
**Construido** (`n8proyecto` en `LEVEL8`, modalidad `proyecto`, commit `37f2ecb`). Extiende la V1 (RAG, heredada de N7 en borrador — ver §3.1) con las capas de Orquestación, Memoria episódica/planning, Multimodalidad y Seguridad/Gobernanza sembradas en M1-M5, cerrando con defensa de arquitectura en rol Arquitecto sobre los 4+ ADRs producidos en M1.T3, M2.T5, M3.T4 y M4.T3. 5 hitos (propuesta de arquitectura → núcleo agéntico/orquestación → memoria/planning → multimodalidad/fiabilidad medida → cierre y defensa), `flujoDeGit` con disciplina de rama+PR por hito (2-4), `dominiosSugeridos` deliberadamente genéricos (dogfooding sobre proyectos reales del propio estudiante) para no comprometer el diseño agnóstico de dominio. Checklist y defensa mapeados 1:1 a C-N8-01…04 — ver detalle completo en el objeto real de `index.html`, no se duplica aquí.

## 7. Compuertas — cobertura de competencias y banco de examen

### 7.1 Tabla de cobertura de competencias

| Competencia | Texto (DOC-01) | Módulo(s) que la cubren | Instrumento principal | ¿Cobertura completa? |
|---|---|---|---|---|
| C-N8-01 | Construye un sistema agéntico — herramientas, memoria, orquestación — que resuelve tareas de varios pasos de forma fiable | M1 (tool calling), M2 (orquestación), M3 (memoria/planning) | Laboratorios integradores M1.T5/M2.T5/M3.T4 + hitos 2-3 del proyecto | Sí — las 3 piezas (herramientas, memoria, orquestación) tienen laboratorio integrador propio antes de exigirse juntas en el proyecto |
| C-N8-02 | Diseña la arquitectura completa de su sistema (componentes, interfaces, datos, fallos) y la defiende ante examen | M1.T3 (primer ADR), M2.T4-T5 (patrones de coordinación), proyecto h5 (defensa) | 4+ ADRs reales producidos en laboratorio + defensa oral del hito 5 | Sí — el ADR no es un ejercicio de escritura aislado, cada uno se produce sobre una decisión de arquitectura real ya implementada |
| C-N8-03 | Anticipa los modos de fallo característicos de los agentes — bucles, herramientas, costes — y los mitiga en su sistema | M5 completo (T1 taxonomía, T2 límites duros, T3 HITL) | Laboratorio integrador M5.T5 (bug real diagnosticado y corregido) + hito 4 del proyecto (métricas reales) | Sí — M5.T5 exige repetir el ciclo completo con un SEGUNDO bug distinto sin guía, evidencia de dominio transferible, no memorización de un caso |
| C-N8-04 | Evalúa los riesgos de mal uso de su sistema y declara sus límites e incertidumbre con honestidad | M5.T4 (auditoría de riesgos propia) | Declaración de límites verificada con al menos un intento real de reproducir un riesgo + hito 5 del proyecto | Sí — la verificación empírica de M5.T4 paso 4 evita que la declaración de límites sea solo aspiracional |

**Hallazgo de la propia revisión (honestidad, no forzada):** ninguna competencia de N8 depende de un solo módulo — las 4 se refuerzan en al menos 2 puntos distintos del nivel (un laboratorio de módulo + el proyecto), a diferencia de lo que la auditoría de SYL-N2 encontró para C-N2-07 (cobertura de un solo módulo, sin refuerzo). Se declara este resultado explícitamente en vez de darlo por sentado.

### 7.2 Banco de examen rotable (NNR-02)

Mismo patrón institucional que N3 (`docs/informes/n3-auditoria-integral.md`): 8 ítems (2 por competencia) × 3 variantes cada uno = **24 variantes**, todas trabajadas y verificadas por razonamiento explícito antes de fijarse aquí — nunca de memoria. A diferencia de N3 (variantes numéricas de cálculo matemático), las variantes de N8 rotan **parámetros de escenario** (tamaño de catálogo, ventana de detección, composición de riesgos) sobre un criterio de arquitectura fijo y verificable — el formato correcto para una competencia de diseño de sistemas, no de cómputo cerrado.

**C-N8-01 · Ítem 1 — encadenamiento de herramientas con dependencia real**
*Enunciado base:* "Tu sistema tiene un catálogo de N herramientas. Una tarea requiere M pasos donde el resultado del paso k es un argumento necesario del paso k+1. ¿Qué patrón de ejecución aplica (secuencial dependiente vs. paralelo independiente) y por qué el patrón contrario produciría un fallo?"
- Variante A: N=5, M=3 (ej. buscar→calcular→guardar). Respuesta correcta: secuencial dependiente — paralelizar el paso 2 antes de tener el resultado del paso 1 produce un argumento inexistente o placeholder.
- Variante B: N=8, M=2, pero los dos pasos NO comparten datos entre sí (ej. dos búsquedas independientes que solo se combinan al final). Respuesta correcta: aquí SÍ cabe paralelismo entre los dos pasos, seguido de un tercer paso de combinación — la pregunta trampa correcta es notar que M=2 con independencia real invierte la respuesta de la Variante A.
- Variante C: N=4, M=4 con dependencia parcial (paso 3 depende de 1 y 2, pero no de forma que necesite ambos completos — solo el más rápido). Respuesta correcta: patrón mixto, con el criterio explícito de qué dependencia real existe, no una regla fija de "siempre secuencial" ni "siempre paralelo".

**C-N8-01 · Ítem 2 — consolidación de memoria**
*Enunciado base:* "Un historial tiene X episodios, de los cuales Y son señal estable y el resto ruido de una sola vez. Antes de responder una pregunta que depende de identificar la preferencia real del usuario, ¿consolidas o no, y con qué política?"
- Variante A: X=20, Y=2 (mucho ruido, poca señal). Respuesta correcta: consolidar es crítico — sin curar, la señal (10% del historial) compite en desventaja severa contra el ruido.
- Variante B: X=20, Y=18 (poco ruido, mucha señal estable). Respuesta correcta: la consolidación sigue siendo buena práctica (coste de contexto) pero el riesgo de que el ruido domine la decisión es mucho menor — la urgencia de curar antes de responder es distinta a la Variante A, aunque la política técnica es la misma.
- Variante C: X=5, Y=5 (historial corto, toda señal). Respuesta correcta: consolidación no aporta valor real todavía — el coste de implementarla no se justifica con tan poco volumen; reconocer cuándo NO consolidar es parte de la competencia, no solo cuándo sí.

**C-N8-02 · Ítem 1 — detectar acoplamiento de proveedor filtrado**
*Enunciado base:* "Se te presenta un fragmento de sistema con K líneas de código de aplicación que mencionan un nombre de proveedor concreto (Anthropic/OpenAI) fuera de la carpeta de adaptadores. Identifica los puntos de acoplamiento y el refactor mínimo."
- Variante A: K=1 (un solo `if proveedor == "anthropic"` en la lógica de negocio). Respuesta correcta: mover la rama condicional dentro del adaptador correspondiente; el código de aplicación no debería necesitar preguntar qué proveedor es.
- Variante B: K=3, con acoplamiento en 3 capas distintas (herramienta, memoria, orquestación). Respuesta correcta: cada punto se resuelve por separado dentro de SU capa — no hay un único refactor central, la solución está distribuida donde vive cada acoplamiento.
- Variante C: K=1, pero el acoplamiento es una capacidad genuinamente exclusiva de un proveedor (ej. MCP). Respuesta correcta: aquí el acoplamiento NO se refactoriza — se documenta como excepción justificada (mismo principio que `n8-arquitectura-plataforma.md` §2), la competencia incluye reconocer cuándo el acoplamiento es correcto, no solo cuándo es un defecto.

**C-N8-02 · Ítem 2 — clasificar patrón de coordinación desde el log**
*Enunciado base:* "Un log muestra 2 agentes colaborando en una tarea. Decide si el sistema usa supervisor o swarm, y qué evidencia específica del log lo confirma."
- Variante A: el log muestra que TODO handoff pasa por un tercer conjunto de mensajes de un agente central antes de llegar al siguiente especialista. Respuesta correcta: supervisor — la evidencia es el punto único de paso.
- Variante B: el log muestra un agente transfiriendo control directamente a otro, sin ningún tercer agente intermedio. Respuesta correcta: swarm — la evidencia es la ausencia de intermediario.
- Variante C: el log es ambiguo — parece haber un agente central pero en un tramo dos agentes se transfieren control directamente sin pasar por él. Respuesta correcta: sistema híbrido o mal etiquetado — la competencia incluye reconocer cuándo la evidencia no encaja limpiamente en ninguno de los dos patrones puros, y decir eso explícitamente en vez de forzar una clasificación.

**C-N8-03 · Ítem 1 — calibrar límites de contención (numérico, verificado)**
*Enunciado base:* "Un agente ejecuta una secuencia de N pasos totales. La misma llamada de herramienta (idéntico nombre+argumentos) se repite en las posiciones dadas. Con una ventana de detección de no-progreso de tamaño W (activa si la misma llamada aparece ≥2 veces dentro de los últimos W pasos), ¿en qué paso se activa la detección? ¿Un recursion_limit de valor L habría contenido el problema antes, después, o al mismo tiempo?"
- Variante A: repeticiones en pasos 3 y 5, W=3, L=10. Verificación: en el paso 5, la ventana de los últimos 3 pasos es [3,4,5] — la llamada del paso 3 SÍ cae dentro de esa ventana, así que la detección se activa en el paso 5. L=10 no se habría alcanzado todavía — la detección de no-progreso actúa primero.
- Variante B: repeticiones en pasos 2 y 9, W=3, L=10. Verificación: en el paso 9, la ventana de los últimos 3 pasos es [7,8,9] — el paso 2 NO cae dentro de esa ventana, así que la detección de no-progreso NO se activa por esta repetición específica (son demasiado distantes para la ventana elegida) — el sistema seguiría hasta L=10, donde recursion_limit sí actuaría. Esta variante existe deliberadamente para mostrar que una ventana mal calibrada puede no atrapar una repetición real.
- Variante C: repeticiones en pasos 4, 6 y 8 (tres veces la misma llamada), W=5, L=10. Verificación: en el paso 6, ventana [2..6] contiene el paso 4 — se activa en el paso 6, mucho antes de L=10.

**C-N8-03 · Ítem 2 — clasificar fallo contra AgentErrorTaxonomy**
*Enunciado base:* "Un log muestra un fallo con estas características [variable]. Clasifica la dimensión primaria (memoria/reflexión/planificación/acción/sistema) y decide si es candidato a IAL."
- Variante A: el agente pide la misma herramienta con argumentos IDÉNTICOS 4 veces seguidas sin ningún cambio de estado entre intentos. Respuesta correcta: dimensión "acción" (o "reflexión" si el propio agente se autoevalúa "LISTO" incorrectamente entre intentos) — SÍ es candidato a IAL si la repetición emerge de la interacción agente-framework-entorno, no de un bug de una sola función.
- Variante B: el agente olvida una preferencia que el usuario ya había declarado 2 turnos antes, en la MISMA conversación (mismo thread_id). Respuesta correcta: dimensión "memoria" — no es candidato a IAL (no hay repetición de acción sin progreso, es una pérdida de contexto).
- Variante C: el plan inicial asumía que una fuente de datos existía, y falla en el paso 2 al no encontrarla, pero el sistema sigue ejecutando el paso 3 como si el paso 2 hubiera tenido éxito. Respuesta correcta: dimensión "planificación" (falta de verificación del resultado real contra la expectativa, mismo principio de M3.T3) — no es candidato IAL, es un fallo puntual de verificación.

**C-N8-04 · Ítem 1 — clasificar herramientas por reversibilidad y patrón HITL**
*Enunciado base:* "Un catálogo tiene M herramientas con las reversibilidades dadas. ¿Cuáles requieren interrupt(), y con qué patrón (aprobar/rechazar-redirigir/editar)?"
- Variante A: M=3 — leer un archivo (trivialmente reversible), escribir una nota en sandbox (reversible con esfuerzo), enviar un correo real (irreversible). Respuesta correcta: solo la tercera requiere interrupt(); el patrón depende de si los argumentos suelen necesitar ajuste (editar) o la acción es binaria en su totalidad (aprobar).
- Variante B: M=4, con dos herramientas irreversibles distintas — una donde cualquier variación de argumentos cambia el significado (aprobar tal cual es el único patrón seguro) y otra donde el usuario casi siempre quiere ajustar un detalle menor (editar antes de ejecutar es más eficiente). Respuesta correcta: dos puntos de control con patrones DISTINTOS, justificados por la naturaleza de cada una, no el mismo patrón aplicado uniformemente.
- Variante C: M=2, ambas aparentemente reversibles pero una depende de una validación de frontera (M1.T4) que podría fallar. Respuesta correcta: la reversibilidad es condicional a que la validación funcione — si hay duda razonable sobre la validación, un punto de control adicional (aunque más ligero) se justifica igual.

**C-N8-04 · Ítem 2 — redactar declaración de límites desde una lista de riesgos**
*Enunciado base:* "Dada una lista de riesgos con su estado de mitigación [variable], redacta la declaración de límites correspondiente."
- Variante A: 3 riesgos, los 3 con mitigación real verificada. Respuesta correcta: la declaración puede ser relativamente confiada, pero SIGUE debiendo declarar explícitamente qué NO se probó fuera de esos 3 riesgos — no basta con "los riesgos conocidos están mitigados".
- Variante B: 3 riesgos, 1 sin mitigación real. Respuesta correcta: la declaración debe nombrar el riesgo sin mitigar explícitamente, no ocultarlo entre los que sí están cubiertos.
- Variante C: 3 riesgos, mitigación DECLARADA pero no VERIFICADA con un intento real (paso 4 de M5.T4 nunca se hizo). Respuesta correcta: la declaración debe distinguir explícitamente "mitigación implementada" de "mitigación verificada" — son afirmaciones de fuerza distinta, y tratarlas como equivalentes es precisamente el error que M5.T4 existe para prevenir.

**Estado:** pendiente de aprobación formal del Director, mismo patrón que N3.

## 8. Paso 9a — Auditoría de coherencia desde N9 y Herencias Declaradas finales

### 8.1 Coherencia de las Herencias entrantes (de N7, §3.1) — reconfirmadas contra DOC-10 §8

La Herencia entrante declarada en §3.1 ("manejo de API keys con advertencias de coste ya interiorizadas; llamadas a un modelo real con streaming/structured output; RAG operativo de extremo a extremo (V1); evaluación básica de sistemas LLM") se reconfirma, en este paso, contra el alcance APROBADO de N7 en DOC-10 §8 (no una versión congelada real, que todavía no existe): M1 "Inferencia y prompting (APIs de modelos, streaming, structured output, costes)", M2 "RAG completo", M4 "Evals y seguridad" — los tres módulos citados en la Herencia entrante corresponden exactamente a estos tres módulos aprobados. **Coherente, sin hallazgo de discrepancia.** Sigue marcada como "pendiente de confirmación cuando N7 congele" — este paso no la da por definitiva, solo confirma que el borrador no contradice el plan maestro.

### 8.2 Herencias Declaradas hacia N9 (H-N9-01…06)

*Mismo formato institucional que H-N4-01…06 (SYL-N3 §9) y H-N7-XX — el contrato explícito de qué N9 puede asumir ya dominado, sin volver a enseñarlo, y qué N9 NO puede asumir porque N8 lo dejó deliberadamente fuera de alcance.*

**H-N9-01 · Desacoplamiento de proveedor de modelo, ya instalado como hábito, no como teoría.** El estudiante ya implementó y probó la interfaz `ModelProvider` con 2+ adaptadores reales (M1.T3-T5, proyecto h2) — N9.M2 (model serving con vLLM/Ray) puede construir el adaptador de modelo local directamente sobre esta misma disciplina de inversión de dependencias, sin tener que instalar el principio desde cero. El propio M1.T3 ya anticipó explícitamente esta extensión en su desafío final.

**H-N9-02 · Medición empírica de coste/latencia como hábito, no como instrucción nueva.** El estudiante ya midió coste real (M1.T1: tokens workflow vs. agente; M4.T1: coste de imagen por complejidad) y latencia por componente (M4.T2: STT/agente/TTS desglosados) antes de optimizar. N9.M2-M4 (latencia/throughput medidos, trade-offs de escalabilidad "con mediciones propias" — C-N9-02, C-N9-03) puede asumir este hábito ya instalado, no como una habilidad nueva a enseñar desde cero.

**H-N9-03 · Auditoría de riesgos y declaración honesta de límites, ya practicada sobre sistema propio.** El estudiante ya auditó su propio sistema con evidencia verificada, no solo declarada (M5.T4, C-N8-04). N9's C-N9-04 ("examina una arquitectura ajena... produce un diagnóstico fundamentado de fortalezas y riesgos") es la misma competencia aplicada a un sistema que el estudiante NO construyó — N9 puede asumir el hábito de auditoría honesta ya instalado y enfocarse en la dificultad nueva genuina (diagnosticar sin haber diseñado).

**H-N9-04 · Orquestación con estado persistente — pero de UN SOLO proceso, brecha declarada explícitamente, no asumible como resuelta.** El estudiante ya orquesta procesos con checkpointing y memoria real (M2-M3), pero exclusivamente con `SqliteSaver` (persistencia de un solo proceso/disco local) — la nota de herencia saliente ya añadida en M2.T2.evalu.cierre (auditoría integral, corrección 2) declara esto explícitamente. **N9.M1 (Kubernetes, múltiples réplicas) NO puede asumir que el estudiante ya sabe operar estado compartido distribuido entre procesos — es una brecha real y con nombre, no una laguna silenciosa.** N9 debe enseñar esto desde cero, con la ventaja de que el estudiante ya entiende POR QUÉ hace falta (vivió el límite real de `SqliteSaver` en M2.T2), no solo QUÉ hacer.

**H-N9-05 · Contención de fallos a nivel de aplicación, no a nivel de infraestructura — misma lógica de brecha declarada.** El estudiante ya implementa `recursion_limit`, detección de no-progreso y HITL (M5.T2-T3) — mecanismos de contención dentro del código de la aplicación. N9.M1/M4 (autoscaling, límites de recursos de contenedor, cuotas de infraestructura) introduce un nivel de contención distinto y complementario, no una repetición — N9 no debe asumir que el estudiante ya conoce estos mecanismos de infraestructura, pero sí puede asumir que entiende el PRINCIPIO general de "límite duro que no depende del buen juicio del sistema", transferible de aplicación a infraestructura.

**H-N9-06 · Protocolos de interoperabilidad entre agentes de organizaciones distintas — explícitamente NO cubierto, brecha declarada en M2.T4.** A diferencia de MCP (herramientas/datos, cubierto con profundidad en M2.T3), N8 declaró explícitamente fuera de alcance los protocolos de comunicación agente-a-agente entre organizaciones distintas (ver corrección de auditoría integral, `n8m2t4.ficha.fuera`). Si el plan maestro de N9-N12 en algún punto asume esta competencia, **no puede asumirla de N8** — queda como hallazgo abierto para quien diseñe el nivel donde corresponda (probablemente N12, gobernanza multiagente empresarial), documentado aquí para que no se pierda en la transición.

**Nota de proceso:** estas Herencias se declaran contra el alcance APROBADO de N9 en DOC-10 §8 (Kubernetes, vLLM/Ray, Kafka, observabilidad — competencias C-N9-01…04), no contra una versión congelada de SYL-N9 (que no existe, N9 se construye en paralelo). Mismo principio que rige toda esta guía: el Desarrollo no espera a la Validación.

## 9. Cierre institucional (2026-07-22)

Flujo de 9 pasos completo. El Director revisó el Informe Final de Nivel y fijó los criterios de cierre directamente, sin abrir una nueva ronda de desarrollo ni de auditoría:

1. **Carga estimada** — corregida con evidencia real (PATCH DOC-10 1.0.4, §1 de este documento), sin inflar la cifra para sostener una expectativa histórica no auditada.
2. **Verificación empírica de los 8 `errorVivo` dependientes de LLM real** — permanece como limitación conocida y transparente, declarada explícitamente en el contenido y en este documento, diferida al entorno definitivo. El Director prefirió esta honestidad a una verificación ficticia.
3. **Ninguna de las dos reservas bloquea la congelación del nivel** — quedan registradas como limitaciones conocidas, no como defectos ocultos.
4. **Propagación de las decisiones institucionales** (plataforma-no-aplicación, agnosticismo de proveedor) — el Director confirmó que no corresponde resolverse desde esta ventana; queda para el documento maestro o la ventana coordinadora, porque afecta a varios niveles simultáneamente.

**SYL-N8 queda candidato a v1.0.0, sin reservas bloqueantes.** Sigue en la rama `nivel/n8`, sin fusionar a `main` — la fusión y el sello final de "v1.0.0" siguen siendo actos exclusivos del Director desde la ventana coordinadora (§11, §12 de la guía maestra).

## 10. Expansión de práctica post-cierre (2026-07-22)

Tras el cierre institucional (§9), el usuario pidió explícitamente profundizar cada tema: *"por cada tema, 15 a 20 días y entre 25 ejercicios"*. Se confirmó con el usuario (pregunta de clarificación) que la forma correcta de honrar esa petición sin romper la naturaleza DOC-12 del nivel (sin `día`/`ejercicio` autoevaluado como en DOC-11) era expandir cada laboratorio existente con un banco grande de práctica — no inventar un esquema de "días" ajeno al estándar de entorno real que rige N8.

**Qué se construyó:** un campo nuevo, `practicaExtendida`, añadido a los 22 objetos `modalidad:"real"` de `LEVEL8` (el capstone `n8proyecto` no lo lleva). Cada banco tiene exactamente 25 ejercicios graduados en 4 niveles — 8 Básico, 9 Intermedio, 5 Avanzado, 3 Integrador —, con `{n, t, nivel, tarea, verificacion}` por ejercicio: una tarea concreta y una verificación explícita, nunca la solución regalada. 550 ejercicios nuevos en total.

**Cómo se construyó:** módulo por módulo (M1→M5), cada laboratorio recibió su banco leyendo el objeto completo del laboratorio (ficha, contexto, pasos, desafío) para que los ejercicios extiendan genuinamente ese contenido — nuevos dominios de aplicación, mediciones que el laboratorio no pidió, combinaciones con laboratorios previos, y al menos un ejercicio Integrador por laboratorio pensado como evidencia reutilizable en la defensa de arquitectura del capstone. Verificado tras cada módulo: `node --check` sobre el script completo, carga de `LEVEL8` en sandbox de Node confirmando 25 ítems por banco con la distribución 8/9/5/3 y numeración `n` secuencial, y unicidad de `id` en todo el Campus (1231 ids, sin duplicados nuevos). 5 commits, uno por módulo.

**Qué NO cambió en esta expansión:** el contenido teórico de los 22 laboratorios (`ficha`, `contexto`, `explicacion`, `pasos`, `recursos`) — la elevación académica con fuentes reales ya se hizo en las versiones 0.5.0-0.6.0 de este documento, antes del cierre institucional. Esta expansión es puramente de práctica adicional.

**Consecuencia declarada sobre la carga estimada:** PATCH DOC-10 1.0.5 (ver §1 y el historial de este documento) — el banco de 550 ejercicios se estima, por categoría de dificultad, en ≈194-338h adicionales (punto medio ≈266h), sumadas al núcleo ya verificado (labs+proyecto, ≈169h) para un total de ≈360-510h (estimado ≈435h, ≈4 meses a la dedicación de 25h/semana ya convencional en la Academia). **Esta estimación de minutos por ejercicio es un supuesto de esta ventana, no una medición empírica con estudiantes reales** — misma naturaleza que la reserva de verificación empírica de `errorVivo` ya registrada en §9; se declara el rango completo en vez de un único número falsamente preciso, siguiendo el mismo principio de honestidad que rigió el PATCH 1.0.4.

Los informes de auditoría integral y adversarial (`docs/informes/n8-auditoria-integral.md`, `docs/informes/n8-auditoria-adversarial.md`) preceden esta expansión y evalúan la versión de 170h/sin `practicaExtendida` — sus hallazgos sobre el contenido teórico y la arquitectura del nivel siguen vigentes (esta expansión no tocó ese contenido), pero su cifra de carga estimada citada en el cuerpo del informe está superada por §1/§10 de este documento y por DOC-10 1.0.5.
