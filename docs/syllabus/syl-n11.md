...
- **Prerrequisitos:** ninguno de N11 — presupone que el estudiante ya sabe leer documentación técnica en inglés (H heredada de N1.M6.T2 — "integra una API pública real leyendo su documentación en inglés sin tutorial mascado", C-N1-07 — y de la pista transversal 🇬🇧 de N1 §7) y arquitecturas reales de sistemas de IA (ET4 completa). *(Corrección de la auditoría adversarial, §11: la cita original a "N1.M1.T7" no correspondía a ningún id real del Campus — no existe tema T7 en N1.M1. Verificado y sustituido por la referencia real.)*
...
- **Práctica principal:** laboratorio real — revisión crítica estructurada sobre el checklist real de NeurIPS (adaptado como instrumento de lectura, no de escritura) aplicado retroactivamente a un paper ya publicado. *(Profundización 2026-07-22, corregida el mismo día tras la segunda auditoría adversarial: añade una mini-review real en los 4 criterios del formulario oficial de revisión de NeurIPS — originalidad/calidad/claridad/significancia — más el criterio real de ICLR ("fracción sustancial de asistentes interesada"), depurada explícitamente de preferencias personales disfrazadas de crítica técnica. La atribución original de los 4 criterios a ICLR era incorrecta — corresponden al formulario de NeurIPS, verificado por WebSearch/WebFetch directo.)*
...
- **Práctica principal:** laboratorio real — redacción estructurada + autoevaluación contra el checklist. *(Profundización 2026-07-22: declara explícitamente si el trabajo es "Reproducción" o "Replicación" según la definición formal de ACM/NISO — corrige el uso coloquial de "reproducir" usado hasta este punto.)*
...
- **Práctica principal:** laboratorio real — redacción + verificación cruzada de atribución/licencias reales del código/datos usados en M2. *(Profundización 2026-07-22: cita licencias con identificadores SPDX estándar; añade checkpoint de divulgación honesta de asistencia de IA usada durante el proceso.)*
...
- **Práctica principal:** laboratorio real — ciclo completo de revisión por pares sobre el reporte de M2/preliminar del capstone. *(Profundización 2026-07-22: examina un caso real de honestidad científica — un RCT de 2023 que NO encontró evidencia de sesgo de anclaje en revisión de CS/ML, pese a ser intuitivo — para enseñar a actualizar creencias ante evidencia, no a asumir sesgos "lógicos" sin verificarlos.)*
...
## 8. Herencia entrante (parcialmente congelada — N1/N3 confirmados; N4-N10 en construcción paralela, borrador)

De N1.M4 (ya congelado, v1.0.0): el flujo Git/PR técnico ya está dominado en contexto controlado — M4 lo extiende a terceros reales, no reenseña sintaxis de Git. De N3.M4 (ya congelado, candidato a v1.0.0): la disciplina de traducir notación de papers a código ya está instalada — M1.T2-T3 la extienden, no la repiten desde cero. **De N6.M4 y de ET4 completa (N4-N10): ninguno de estos niveles está congelado todavía — se construyen en ramas paralelas al mismo tiempo que N11 (mismo escenario que guia-construccion-niveles.md §13 describe explícitamente).** Las herencias de N6.M4 (notación de frontera específica de Transformers/Deep Learning) y de ET4 completa (sistemas de IA operados de extremo a extremo) se tratan aquí como borrador basado en el alcance ya aprobado de DOC-10 para esos niveles, no en contenido real ya verificado — **pendiente de confirmación cuando N4-N10 congelen sus Herencias Declaradas finales** (mismo principio de SYL-N3 §7: no bloquea el avance de este syllabus). *(Corrección de la auditoría adversarial, §11: la versión anterior de este párrafo aplicaba el hedge de "no congelado" solo a N7-N10, dejando fuera a N6 pese a estar en la misma situación real de construcción paralela no congelada.)*
...
**Nota sobre DOC-11 §4bis — actualizada tras construir el contenido real (corrección de la auditoría adversarial, §11).** Esta nota, escrita durante el Paso 1 (antes de que existiera contenido real), decía que "no hay campo `recursos` inline en el objeto día" — cierto para N3 (Pyodide, DOC-11), pero **N11 sí lo tiene**: cada uno de los 17 laboratorios `real` construidos en `index.html` incluye un campo `recursos:[...]` inline (2-3 fuentes por laboratorio, clasificadas 🟢/🔵/🟣, con los 7 campos que exige DOC-12 §2.12), visible directamente al estudiante dentro del Campus. N11 cierra por su cuenta la brecha que la guía maestra dejaba pendiente desde N3 — pero solo para su propia modalidad `real` (DOC-12 §2.12 ya lo exige explícitamente como sección obligatoria); la ambigüedad de DOC-11 §4bis para contenido `pyodide` (si N11 llegara a tener alguno) sigue sin resolver y se hereda tal cual a quien construya niveles Pyodide en el futuro.

## 11. Paso 9b · Auditoría Adversarial de SYL-N11 (mandato: demostrar que N11 no debería existir en su forma actual)

**Método:** 4 auditores independientes, sin coordinación entre sí (mismo patrón que N1/N2/N3), cada uno con un ángulo de ataque distinto sobre el contenido REAL ya construido (`index.html`, 18 objetos) y el syllabus completo — no sobre la intención de diseño. Mandato explícito: atacar la arquitectura completa, no buscar mejoras cosméticas.

### Ataques que el diseño resistió

1. **¿Los 17 laboratorios son una plantilla rellenada, sin sustancia real distinta entre sí? (Auditor B)** Comparación línea por línea de 9 laboratorios completos: el esqueleto de 13 secciones es idéntico porque DOC-12 lo exige como estándar, no como elección de autor — pero el contenido dentro de esa estructura no se repite (método de Keshav → notación → verificación → checklist reutilizado primero como lectura y después como escritura → licencias → revisión por pares → convenciones OSS → triage → PR → feedback). **Resiste.**
2. **¿La decisión de "preparación acotada en el módulo, ejecución real en el capstone" (§3.1) diluye la exigencia en vez de sostenerla? (Auditor C)** Verificado que cada laboratorio de preparación practica el mecanismo EXACTO que su hito correspondiente escala (M2.T4 → hito 2, M4.T2-T4 → hito 3), con los propios campos `evalu.capacidades` de los laboratorios declarándolo explícitamente ("este mismo flujo, a mayor escala, es el hito N del capstone"). El salto es de escala temporal, no de mecanismo desconocido. **Resiste.**
3. **¿La mitigación del riesgo del hito 3 (§6.4, aceptación de PR no garantizada) es una puerta trasera que diluye "contribuciones aceptadas" de DOC-10? (Auditor C)** La política restringe la excepción con precisión ("solo si la demora es atribuible al mantenedor") — pero se encontró una **inconsistencia real** entre esa política y el campo operativo que un tutor usaría para verificarla (ver Hallazgo 3 abajo). **Resiste en su diseño, no resistía en su implementación exacta — ya corregido.**
4. **¿Las citas y fuentes del nivel son inventadas o están desactualizadas, dado que ya se encontró un caso real (Papers With Code) durante la construcción? (Auditor D)** Verificación independiente por WebSearch/WebFetch de 6 citas de módulos distintos (Keshav, Semmelrock et al., MLRC, comparación de licencias, MIT Comm Lab) — todas confirmadas reales y fielmente representadas, incluida la disciplina de declarar honestamente lo que una fuente NO cubre (MLRC no da criterio de selección de candidato) en vez de rellenarlo. **Resiste — la disciplina de verificación de este módulo es más estricta que el promedio del proyecto auditado.**
5. **¿La carga horaria y las promesas de tiempo son poco realistas? (Auditores B y D)** Las cifras usan el mismo formato hedgeado que N1-N3, y varias declaran explícitamente su propia incertidumbre (capstone: "variable — depende de terceros reales"). La cifra de DOC-10 (~600h) se marca "no auditada" en vez de prometerse. **Resiste.**
6. **¿Falta un banco de examen, violando el estándar institucional? (Auditor A)** No — DOC-10 §9 autoriza explícitamente que el examen de N11 ES la reproducción y su defensa. **Resiste.**

### Hallazgos reales — aplicados

**Hallazgo 1 (moderado, Auditor A) — cita de herencia fabricada.** `syl-n11.md` e `index.html` (M1.T1) citaban "N1.M1.T7, pista 🇬🇧" como fuente de la habilidad de leer documentación técnica en inglés — **verificado por el auditor y confirmado por mí de forma independiente (grep directo sobre `index.html`): no existe ningún id `n1m1t7` en todo el Campus.** N1.M1 usa el esquema `n1m1t6a`/`t6b` (módulos, entornos virtuales); la pista 🇬🇧 es transversal (SYL-N1 §7), sin lección numerada propia. **Aplicado:** sustituido por la referencia real y verificable — N1.M6.T2 (C-N1-07, "integra una API pública real leyendo su documentación en inglés sin tutorial mascado") + la pista transversal, en ambos archivos.

**Hallazgo 2 (moderado, Auditor B) — error inducido en vivo no determinista en 2/17 laboratorios.** M1.T1 (comparar versiones de un paper) y M1.T2 (símbolo reutilizado) dependen parcialmente de qué paper elige el propio estudiante — a diferencia de un comando de terminal, no hay garantía del 100% exigida literalmente por DOC-12 §2.5bis. **Aplicado:** M1.T2 ahora ancla el error a un caso garantizado dentro de la propia Academia (θ ya cambia de significado entre N3.M4.T1e8 y su uso típico en probabilidad, verificable sin depender del paper elegido); M1.T1 declara explícitamente que "no encontrar diferencia" es un resultado válido, no un fallo. La adaptación parcial de DOC-12 §2.5bis para contenido dependiente de fuentes externas no curadas queda documentada en `docs/investigacion/n11-m1-lectura-de-frontera.md`, no oculta.

**Hallazgo 3 (moderado, Auditor C) — inconsistencia entre la `evidenciaLabel` del hito 3 del capstone y la política real de §6.4.** La `evidenciaLabel` original permitía marcar el hito cumplido con "cerrado con causa documentada" sin repetir la condición restrictiva (que la causa sea atribuible al mantenedor, no a la calidad del PR) — un tutor que solo consultara ese campo operativo podría aprobar un PR rechazado por mala calidad. **Aplicado:** `evidenciaLabel` reescrita en `index.html` para incluir la misma condición restrictiva explícitamente.

**Hallazgo 4 (menor, Auditor C) — la "integración" del informe del hito 4 podía ser solo de formato.** El hito 3 solo pedía relación "idealmente" temática entre la contribución OSS y el paper reproducido, sin exigirlo ni pedir justificación si no se cumplía. **Aplicado:** el hito 3 ahora prioriza explícitamente un proyecto relacionado, y si no es viable, exige justificación explícita en el informe del hito 4 — evita que la "síntesis integrada" sea solo dos secciones cosidas.

**Hallazgo 5 (menor, Auditor D) — nota obsoleta sobre `recursos` inline (§10/DOC-11 §4bis).** La nota, escrita en el Paso 1 antes de que existiera contenido real, seguía afirmando que N11 no tenía campo `recursos` inline — falso una vez construido el contenido: los 17 laboratorios sí lo tienen (DOC-12 §2.12 lo exige). **Aplicado:** nota reescrita en §10 reconociendo que N11 cierra esa brecha por su cuenta, con el alcance real de la corrección (solo modalidad `real`, DOC-11/Pyodide sigue sin resolver).

**Hallazgo 6 (moderado, Auditor A) — vacío pedagógico: ninguna mención de divulgación de asistencia de IA en investigación real.** Para un nivel de "Research Engineer" construido en 2026, sobre un estudiante que llega de ET4 (LLM Engineering), la ausencia total de tratamiento de cuándo/cómo declarar el uso de asistentes de IA en lectura, código o redacción es una omisión real, coherente con C-N11-04 (honestidad/atribución) pero no cubierta por ella explícitamente. **Aplicado:** M3.T2 (atribución y licencias) ahora incluye un checkpoint verificable (`cp4`), una pregunta de comprensión, y una buena práctica explícita sobre declarar asistencia de IA con la misma disciplina que una licencia.

**Hallazgo 7 (menor, Auditor A) — hedge de "herencias no congeladas" aplicado inconsistentemente.** Cubría N7-N10 pero no N6, pese a que ambos son ramas paralelas sin congelar en el mismo momento. **Aplicado:** §8 reescrita para distinguir explícitamente entre herencias de niveles ya congelados (N1, N3 — citadas con confianza) y herencias de niveles en construcción paralela (N4-N10, incluido N6 — declaradas como borrador, no bloqueantes).

**Hallazgo 8 (menor, Auditor D) — la cita de Keshav depende de una fuente secundaria.** El PDF original no pudo fetchearse directamente (dos mirrors fallaron por protocolo); el laboratorio T1 se apoya en un resumen de Substack verificado como fiel pero de menor autoridad institucional. **Resuelto (2026-07-22):** un tercer mirror (Stanford CS114, `web.stanford.edu/class/cs114/reading-keshav.pdf`) sí cargó — fetch directo confirmó textualmente las 5 C's y los tiempos exactos de cada pase, sin contradicción con la síntesis previa. El recurso citado en M1.T1 se actualizó a este mirror verificado.
...
## 13. Cierre (de este paso, no del nivel)
