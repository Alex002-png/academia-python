...
| **Versión / Estado** | **1.0.0-candidato · con 3 reservas nombradas** · Flujo institucional de 9 pasos completo (`docs/guia-construccion-niveles.md` §13): investigación pedagógica + syllabus (Paso 1) → M1-M4 construidos en `index.html`, 30 días/laboratorios (Paso 2-3) → capstone `n7et4` + compuertas + banco de examen (Paso 4-5) → Paso 9a (auditoría de coherencia desde SYL-N8 real + Herencias Declaradas finales, §7) → auditoría integral con 4 auditores independientes (`docs/informes/n7-auditoria-integral.md`, 10 hallazgos: 5 aplicados, 3 declarados reserva nombrada, 2 informativos cross-nivel) → Informe Final de Nivel (`docs/informes/n7-informe-final-de-nivel.md`). **Reservas abiertas** (§6.5-6.6): reconciliación de horas contra DOC-10 sin resolver (~140h identificadas de ~400h estimadas); prompt engineering subrepresentado como disciplina evaluada; M1.T1 sin alternativa de proveedor sin tarjeta. Ninguna bloquea que un estudiante complete N7 preparado para N8. A la espera del veredicto definitivo del Director. |
...
1. **Prompt engineering como disciplina propia (few-shot, chain-of-thought, iteración sistemática de prompts) queda relegado a lectura opcional 🔵, nunca a ejercicio evaluado** — hallazgo convergente de dos auditores independientes (cobertura de competencias y adversarial). M1 se llama "Inferencia y **prompting**" pero los 7 laboratorios reales son, en su mayoría, mecánica de API (contrato, streaming, historial, tool calling, structured output, costes) — sin ejercicios dedicados a técnicas de prompting como objeto de práctica evaluada en sí mismas. Ninguna competencia C-N7-01…05 lo exige explícitamente (no es una violación de DOC-01), pero es un vacío real frente a lo que un practicante de LLM Engineering necesitaría. **No se corrige en esta pasada** — se declara como reserva nombrada para que el Director decida si amerita un laboratorio adicional en M1 o si se acepta como alcance ya cerrado (DOC-10 §8 no lo exige explícitamente para N7).
2. **M1.T1 no explora un proveedor sin tarjeta de crédito (ej. Google AI Studio, Groq, OpenRouter) como alternativa real a Anthropic/OpenAI** — hallazgo del auditor adversarial. La decisión de "key propia desde el día 1" está bien justificada pedagógicamente (`docs/investigacion/n7-m1-manejo-credenciales.md`), pero el documento nunca considera seriamente una alternativa que reduciría la exclusión de estudiantes sin método de pago, sin sacrificar el principio de credenciales reales. **No se corrige en esta pasada** — se declara como reserva nombrada, recomendación concreta para una futura revisión de M1.T1: mencionar explícitamente al menos una alternativa sin tarjeta en el paso 1 del laboratorio.
3. **H-N8-05 (§7.2): redundancia real de tool calling entre N7.M1.T3 y N8.M1.T2**, no resuelta — pendiente de arbitraje del Director entre ventanas paralelas, no de N7 unilateralmente.

---

*Paso 4-5 completo, con 3 reservas nombradas (§6.5, §6.6) — mismo patrón que SYL-N3 usó antes de su propio veredicto. Ninguna bloquea que un estudiante complete N7 y esté preparado para N8.*
