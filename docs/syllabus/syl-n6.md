...
| **Versión / Estado** | **0.3.0-candidato** · Pasos 1-9 del flujo institucional de 9 pasos completos: diseño de syllabus + M1-M4 construidos y verificados en `index.html` (39 días/objetos, 106 ejercicios Pyodide, 1 laboratorio DOC-12, harness 100% pass en los 4 módulos) + Capstone ET3 (Mini-GPT) + Paso 8 (compuertas, banco de examen) + Paso 9 (Herencias Declaradas finales hacia N7 + auditoría adversarial, sin reservas abiertas). **Pendiente exclusivamente:** Informe Final de Nivel y veredicto definitivo del Director. |
...
**Hallazgo 1 (menor, Beyond the Curriculum) — M3.T3 no menciona variantes de atención más allá de GQA que existen en producción real (atención dispersa, ventana deslizante/sliding window).** No amerita tema propio en N6 (son generalizaciones más avanzadas, correctamente diferidas a N7-N9) — recomendado como mención breve de una línea si se revisa M3.T3 en el futuro, sin nuevo contenido ni carga adicional, mismo patrón que la mención de SVD que la auditoría de N3 aplicó a M1.T7 de ese nivel. **No aplicado en esta pasada** (costo/beneficio no justifica una edición adicional del contenido ya verificado; queda como recomendación explícita, no una promesa incumplida).
