...
- **T1 — modelos de gobernanza y Contributor Covenant:** confirmado por WebSearch con cifras reales y verificables — Contributor Covenant (Coraline Ada Ehmke, 2014; hoy bajo Organization for Ethical Source; v3.0 desde julio 2025) usado por 40,000+ proyectos, incluidos 9 de los 10 OSS más grandes del mundo (Kubernetes, Rails, Swift citados explícitamente). Los modelos de gobernanza (BDFL/fundación/empresa) son categorización propia sintetizada a partir de casos ampliamente documentados (Python bajo Guido van Rossum como caso histórico de BDFL, Apache/Linux Foundation como fundaciones, React/Meta como caso patrocinado por empresa) — no una taxonomía de una única fuente académica, declarado como tal.
- **T2 — Semantic Versioning:** semver.org, especificación oficial 2.0.0 — fuente primaria única, sin ambigüedad, ampliamente adoptada como estándar de facto en el ecosistema de paquetes de software.
- **T3 — Conventional Commits:** conventionalcommits.org, especificación oficial 1.0.0 — reutiliza el hallazgo ya documentado en `n1-m4-git-github.md` (mencionado ahí como "⭐ profundización"), aquí promovido a contenido central porque en M4 de N11 el estudiante se encuentra con proyectos reales que SÍ lo exigen, a diferencia del contexto controlado de N1.
- **T4 — distinción feedback técnico vs. violación de conducta:** reutiliza el Contributor Covenant ya verificado en T1, aplicado ahora a sus ejemplos concretos de comportamiento aceptable/inaceptable — no requiere fuente nueva.

## Clasificación y falsabilidad

**Evidencia sólida:** toda la síntesis original de este documento proviene de una única fuente oficial verificada por WebFetch directo (opensource.guide), sin necesidad de convergencia de múltiples fuentes porque es la fuente canónica del ecosistema (mantenida por GitHub) — mismo estatus de autoridad que la documentación oficial de Git ya usada en N1.M4. Las adiciones de la pasada de profundización (Contributor Covenant, SemVer, Conventional Commits) son, cada una, la especificación oficial única de su estándar respectivo — sin ambigüedad de fuente. **Ausencia declarada:** ninguna fuente cuantifica cuánto tiempo real toma en promedio obtener respuesta de un mantenedor — se documenta esto como incertidumbre real en el propio SYL-N11 §3.3/§6.4, no se inventa una cifra; la taxonomía de modelos de gobernanza (BDFL/fundación/empresa) es síntesis propia sobre casos documentados, no una clasificación de una fuente académica única — declarado explícitamente en T1. **Falsabilidad:** si el registro de ejecución muestra que los estudiantes de N11 encuentran con frecuencia proyectos donde el endpoint `/contribute` no está configurado, se ajusta T2 para priorizar las listas curadas externas como primera vía, no como respaldo.
