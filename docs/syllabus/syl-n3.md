...
| **Versión / Estado** | **1.0.0-candidato** · Flujo institucional de 9 pasos completo (DOC-03 §8bis / EVT-022): diseño + M1-M4 (24/24 temas) + Capstone ET3 + Paso 8 (compuertas + banco de examen) + Auditoría Integral + Informe Final de Nivel + Paso 9 (coherencia desde N4 + Herencias Declaradas finales + auditoría adversarial) completos — **candidato a v1.0.0 sin reservas abiertas** (las 2 reservas del Paso 9b —pistas que regalaban la solución; estándares narrativos sin ruta de renderizado— fueron resueltas por instrucción del Director tras el veredicto inicial del comité), **pendiente del veredicto definitivo del Director**, no autodeclarado |
| **Autoridad de origen** | DOC-10 §6 (interior de N3) · DOC-00 14.8.3 · DOC-01 (C-N3-01…04) |
| **Depende de** | DOC-10 v1.0.3 (mapa, con la bibliografía de M3 ampliada con Seeing Theory y la carga estimada de N3 corregida a ~4,6 meses) · DOC-00 (§13, §14, §16, 17.6) · DOC-01 · DOC-02 (instrumentos) · DOC-03 (método) · **SYL-N2 §15 (Herencias Declaradas H-N3-01…05 — el contrato de entrada que este documento resuelve)** |
...
3. **Hallazgo técnico real que condiciona el diseño de M1 (verificado con ejecución directa, no documentación de terceros):** en esta build de Pyodide, `numpy.linalg.inv()` sobre una matriz singular **NO lanza `LinAlgError`** como en numpy nativo — devuelve silenciosamente una matriz de `nan` (WASM no soporta excepciones de punto flotante de la misma forma que CPython nativo; confirmado también en el issue público pyodide/pyodide#4859). Ninguna lección de M1 puede enseñar "captura el error con try/except" para detectar una matriz singular — la disciplina correcta, y la que este syllabus adopta, es verificar `np.isnan()` en el resultado o comparar `det() == 0` (con tolerancia) ANTES de invertir. Esto se declara explícitamente en la ficha de M1.T5. **Nota añadida tras la auditoría adversarial del Paso 9b:** este hallazgo y el de M3.T3 (`numpy.var()` usa `ddof=0` por defecto, `numpy.cov()` usa `ddof=1` — dos funciones de la misma librería con convenciones distintas) no son dos trivia sueltas — son dos instancias del MISMO principio general: **las implementaciones numéricas reales tienen comportamientos específicos de la herramienta que no siempre coinciden con la definición matemática pura, y hay que verificarlos empíricamente, nunca asumirlos por familiaridad con la fórmula.** Ningún tema de N3 nombra este principio de forma unificada — cada uno lo enseña como hallazgo aislado de su propio contexto, una elección pedagógica válida (cada hallazgo es más memorable anclado a su propio ejemplo concreto) pero que vale la pena que el estudiante conecte explícitamente al llegar al segundo caso.
...
- **Por qué:** existe porque los autovalores/autovectores son el mecanismo exacto detrás de PCA (Curso 3 de la especialización de referencia, y una de las primeras técnicas reales de N4) / ahora porque T3-T6 ya dieron toda la maquinaria necesaria / habilita directamente el proyecto de nivel y N4. **Beyond the Curriculum (añadido tras la auditoría adversarial del Paso 9b, sin nuevo contenido ni carga adicional):** en la práctica real, PCA se calcula casi siempre vía descomposición en valores singulares (SVD) sobre los datos directamente, no vía eigendescomposición de la matriz de covarianza que este tema enseña — SVD es más numéricamente estable y evita construir la matriz de covarianza explícitamente. N3 enseña la vía de covarianza deliberadamente (es la que conecta directamente con M3.T3 y es algebraicamente más simple de derivar a mano); SVD queda, correctamente, diferida a N4+ como una generalización más avanzada.
...
| H-N4-04 | M3: distribuciones, esperanza, inferencia básica | N4.M4 (métricas, splits, overfitting — DOC-10 §6) presupone este vocabulario directamente (un intervalo de confianza sobre una métrica, o "overfitting" leído como sesgo de un estimador, son M3.T4/T3 aplicados); N4.M3 (ML clásico aplicado — scikit-learn: árboles, SVM, ensembles, clustering, según la descripción real de DOC-10 §6) lo usa de forma más indirecta, no como "clasificadores probabilísticos" (corrección de la auditoría integral de N3: la formulación original no coincidía con la descripción real de N4.M3 en DOC-10 — clustering sí conecta con la intuición de distribución de M3.T2, pero el módulo no es, en sí, de clasificadores probabilísticos) |
...
## 9. Paso 9a · Auditoría de coherencia desde SYL-N4 y Herencias Declaradas

*Análisis desde el nivel siguiente: ¿qué presupone la descripción real de N4 (DOC-10 §7, "Machine Learning") que todavía no existe en N3? SYL-N4 no existe todavía como documento propio (misma situación exacta que enfrentó SYL-N2 en su propio Paso 9a, cuando SYL-N3 tampoco existía aún) — este análisis se hace contra la descripción real de DOC-10, no contra un syllabus futuro. Consolidación final de las Herencias provisionales de §7.2, verificadas ahora contra el contenido REAL de M1-M4 y el Capstone ET3 — no contra la intención inicial de diseño.**

| # | N3 sembró (verificado contra contenido real) | N4 deberá recoger |
|---|---|---|
| H-N4-01 | M1 completo: vectores/matrices/transformaciones operados e implementados a mano, verificados contra numpy con tolerancia explícita | N4.M2 ("Modelos desde cero — regresiones y clasificadores con su matemática") reutiliza directamente esta literatura vectorial — no se reintroduce desde cero |
| H-N4-02 | M2.T4: descenso de gradiente derivado e implementado desde cero, con divergencia, mínimos locales vs. globales, y curvatura anisotrópica por dimensión ya explorados | N4.M2 lo reutiliza directamente como el algoritmo de entrenamiento — se aplica, no se re-enseña |
| H-N4-03 | Disciplina de verificar contra numpy en pequeña escala, con tolerancia explícita, ejercitada en los 24 temas de N3 sin excepción | N4.M1 ("NumPy, pandas, EDA... el 80% del trabajo real de ML") usa numpy/pandas en escala real de producción — el hábito ya está instalado; **límite honestamente declarado, no resuelto:** falta enseñar por qué vectorizar importa a escala, algo que Pyodide no puede demostrar limpiamente — nota abierta para el diseño de N4 |
| H-N4-04 | M3 completo: distribuciones, esperanza/varianza/covarianza, inferencia básica, intervalos de confianza | N4.M4 ("Metodología — splits, métricas, overfitting") presupone este vocabulario directamente (un intervalo de confianza sobre una métrica, u "overfitting" leído como sesgo de un estimador, son M3.T4/T3 aplicados sin re-explicar); N4.M3 ("ML clásico aplicado — scikit-learn: árboles, SVM, ensembles, clustering") lo usa de forma más indirecta (clustering conecta con la intuición de distribución de M3.T2) — **no como "clasificadores probabilísticos"**, corrección real aplicada tras la Auditoría Integral: la formulación original de esta fila no coincidía con la descripción real de N4.M3 en DOC-10 §7 |
| H-N4-05 | M4 completo: lectura sistemática de notación matemática de papers y documentación oficial de frameworks (verificado contra PyTorch y scikit-learn reales, no solo teoría) | N4.M2 ("explicando su matemática") presupone que el estudiante ya puede leer la fórmula de un modelo tal como aparece en documentación real, sin bloquearse ante notación desconocida |
| H-N4-06 *(nueva — hallazgo de la Auditoría Integral, no planeado desde el diseño original)* | M4.T2/T5: la convención de notación NO es universal, ni siquiera entre fuentes oficiales de producción — confirmado de forma independiente y no anticipada en dos frameworks reales (PyTorch define su capa lineal como `y=xAᵀ+b`; scikit-learn como `Xᵢw+w₀`; ninguna coincide exactamente con la convención de pizarra `y=Wx+b` que M4 enseña), y reforzado por el propio desafío final del nivel (el símbolo σ significa "desviación estándar" en M3/M4.T4 pero "función sigmoide" en M4.T5) | N4 leerá documentación real de numpy/pandas/scikit-learn con sus propias convenciones específicas — el estudiante ya trae la disciplina de verificar la convención de CADA fuente antes de asumir familiaridad, en vez de sorprenderse o bloquearse cuando una librería usa una notación distinta a la que aprendió en N3 |

**Prerrequisitos ocultos verificados — ninguno encontrado.** N4 (NumPy/pandas a escala, modelos desde cero, ML clásico con scikit-learn, metodología de evaluación, proyecto gestionado) no presupone ningún contenido de N3 que no se haya construido realmente — verificado módulo por módulo contra DOC-10 §7, no solo contra la intención de diseño. La única nota abierta real (H-N4-03: vectorizar a escala) ya está honestamente declarada, no oculta, y no bloquea el inicio de N4 — es una profundización, no un prerrequisito ausente. La cadena N3→N4 del PMC es correcta: N4 no espera de N3 nada que N3 no le dé.

---

## 10. Paso 9b · Auditoría Adversarial Final de SYL-N3 (mandato: demostrar que N3 no debería existir en su forma actual)
...
1. **¿"100% Campus/Pyodide, sin ningún laboratorio de entorno real" deja a N3 mal preparado para la escala real que N4 exigirá?** Objeción real, examinada a fondo. Pero el criterio de frontera de DOC-12 §5 (¿sale del navegador? ¿depende del SO? ¿cuentas externas? ¿estado persistente?) responde "no" a las tres para el proyecto declarado — y Pyodide sí sostiene con solvencia el álgebra lineal, cálculo y estadística que N3 enseña. La única limitación real (por qué vectorizar importa A ESCALA) ya está honestamente declarada como nota abierta (H-N4-03), no oculta ni negada. **Resiste.**
2. **¿M4 debería enseñarse intercalado con M1-M3, en vez de secuencial y al final?** Un estudiante encuentra notación mientras aprende conceptos, no después — objeción real. Pero la razón de diseño original (M4 mezcla símbolos de M1+M2+M3 simultáneamente; no hay nada real que practicar en notación combinada hasta que los tres módulos existan) sigue siendo válida, y esta decisión ya fue deliberada explícitamente con el Director desde el diseño original (§5, historial 0.1.0-draft) — no es una omisión sin examinar. **Resiste.**
3. **¿N3 debería fusionarse con N2 o con N4, en vez de existir como nivel propio?** Se examinó la frontera en ambos sentidos. N2→N3: verificado en el propio Paso 9a de SYL-N2 (§15 de ese documento) que N3 no depende técnicamente de ningún contenido de ingeniería de N2 — la cadena real es N1→N3. N3→N4 (verificado arriba, Paso 9a de este documento): las 4 competencias de N3 son, cada una, un prerrequisito real y activamente usado por al menos un módulo de N4, sin redundancia entre ellas. La frontera de N3 como nivel autónomo es limpia en ambos extremos. **Resiste.**
4. **¿Las 4 competencias (C-N3-01…04) cubren realmente "matemáticas para IA", o falta algo que un practicante real necesitaría?** Se examinó contra un currículo de referencia (estabilidad numérica como idea propia, descomposición SVD, información/entropía, visualización). La mayoría falla la prueba "coste de no enseñarlo ahora": visualización pertenece a N4.M1 (declarado); información/entropía ya tiene una siembra mínima y honesta en M4.T5 (un símbolo nuevo, no un tema); SVD es una generalización de la descomposición espectral que M1.T7 ya enseña, más avanzada de lo que N3 necesita para su propio cierre. **Resiste en general — con un caso límite real** (hallazgo 1, abajo).
5. **¿El aparato de "5 estándares" pedagógicos por módulo (quiebre de intuición, supuesto que destruye, idea universal, lo que deja de sorprender, limitación humana) es sobre-ingeniería documental que no llega a cumplir su función?** Este comité no necesita re-examinarlo desde cero: **la Auditoría Integral de N3 (`docs/informes/n3-auditoria-integral.md`) ya encontró, de forma independiente, que la sustancia de estos 5 estándares SÍ está genuinamente presente en el contenido real, pero su señalamiento explícito no tiene ruta de renderizado en la modalidad Pyodide** — a diferencia de N2, que sí hizo visible su "Momento Fundacional" en títulos y etiquetas. No es sobre-ingeniería documental (el contenido es real y bien construido) — es una brecha entre diseño y visualización, ya documentada como riesgo pendiente en el Informe Final de Nivel. Este comité **confirma el hallazgo ya existente, no lo diluye ni lo eleva.**
6. **¿Falta algo que un ingeniero de IA profesional necesitaría y que N3 no cubre en absoluto?** Examinado contra el mismo currículo de referencia del punto 4. Único caso límite real: la numeración de estabilidad numérica como IDEA UNIFICADORA (hallazgo 1).

### Hallazgos reales — aplicados

**Hallazgo 1 — la estabilidad numérica aparece dos veces como hallazgo aislado (M1.T5, M3.T3) sin nombrarse nunca como una idea unificadora propia.** El riesgo genuino: un estudiante puede memorizar "numpy.linalg.inv() da nan en Pyodide" y "numpy.cov() usa ddof=1 por defecto" como dos trivia desconectadas, sin reconocer que ambas son INSTANCIAS del mismo principio general (las implementaciones numéricas reales tienen comportamientos específicos de la herramienta que no siempre coinciden con la definición matemática pura, y hay que verificarlos, no asumirlos). Aplicado: nota añadida en §3 conectando explícitamente ambos hallazgos como el mismo principio, no dos casos sueltos.

**Hallazgo 2 (menor, Beyond the Curriculum) — M1.T7 no menciona que la descomposición en valores singulares (SVD) es la alternativa más usada en la práctica real para calcular PCA, más numéricamente estable que la eigendescomposición de la matriz de covarianza que el tema enseña.** No amerita tema propio en N3 (SVD es una generalización más avanzada, correctamente diferida) — aplicado como mención breve de una línea en la ficha de M1.T7 (§5), sin nuevo contenido ni carga adicional, mismo patrón que la mención de rate limiting que la auditoría de N2 aplicó a M2.T4 de ese nivel.

### Conclusión formal del comité

Tras el intento deliberado de demostrar que N3 no debería existir en su forma actual: **la arquitectura resiste sin excepción en sus fronteras (con N2 y con N4), en su decisión de plataforma (100% Pyodide) y en su secuencia interna (M4 al final).** Los dos hallazgos reales de esta pasada (estabilidad numérica sin nombre unificador, SVD no mencionado) se aplicaron con costo mínimo, sin nuevo contenido sustancial. **A diferencia de la conclusión de SYL-N2 ("candidato a v1.0.0 sin reservas"), este comité NO puede llegar a esa misma conclusión sin matices**, porque la Auditoría Integral ya identificó dos riesgos reales y no resueltos que este comité confirma en vez de descartar: el patrón de pistas que regalan la solución (79-92% de ~285 ejercicios, tercera vez consecutiva sin corregirse) y la ausencia de ruta de renderizado para los 5 estándares narrativos. Ninguno de los dos es un defecto de arquitectura (ambos son defectos de ejecución dentro de una arquitectura sólida) y ninguno bloquea que un estudiante complete N3 y esté preparado para N4 — pero ambos son reales, ya documentados dos veces (Auditoría Integral + Informe Final de Nivel), y merecen una decisión explícita del Director antes de la publicación, no un silencio que los deje pendientes indefinidamente. **SYL-N3, con las correcciones ya aplicadas en esta pasada y con esas dos reservas explícitamente registradas, queda declarado por este comité candidato a v1.0.0 — no sin reservas, sino con reservas nombradas y acotadas.**

**Actualización posterior (2026-07-21, misma jornada) — ambas reservas resueltas por instrucción directa del Director ("resuelve, y súbelo").** (1) Las pistas de los 132 ejercicios reales que regalaban la solución completa se reescribieron progresivamente (hint conceptual específico antepuesto, código preservado como pista posterior) mediante 8 agentes en worktrees git aislados — uno por porción de módulo, para permitir edición paralela del mismo archivo sin conflictos — fusionados sin ninguna colisión real, verificados sin regresión en los 4 harnesses de módulo (M1 85/85, M2 72/72, M3 72/72, M4 55/55) y en `node --check` sobre el bloque `<script>` completo. (2) Los 5 estándares narrativos se hicieron visibles en el contenido real: "el quiebre de intuición" en el título de la tema donde ocurre por módulo (M1.T3, M2.T4, M3.T1, M4.T5), y los 4 restantes (idea universal, lo que deja de sorprender, limitación humana que compensa, el supuesto que destruye) añadidos a la prosa de `intro` de los 4 cierres reales de módulo — decisión de diseño resuelta a favor de "disciplina de prosa" sobre "campo de datos nuevo", la segunda opción que el propio Paso 9b había dejado abierta. **Con esto, SYL-N3 no tiene ninguna reserva abierta pendiente de decisión — queda candidato a v1.0.0 sin matices, a la espera exclusivamente del veredicto formal del Director.**

---

*Con el Paso 9 completo (coherencia desde N4 + Herencias Declaradas finales + auditoría adversarial), SYL-N3 queda a la espera del veredicto definitivo del Director para su publicación como v1.0.0.*
